...
conn = MySQLdb.connect(host='47.100.114.71', port=3306, user='jklf5',
                       passwd='jklf5678', db='movieInfo', charset='utf8',)  # 连接数据库
cur = conn.cursor()


def getList(page):
    html = ulr.urlopen(
        'http://www.ygdy8.net/html/gndy/dyzz/list_23_%s.html' % page)
...
for i in range(1, 157):
    for url, title in getList(page=i):  # getList()=列表[(url,标题),(url2,标题2)]
        ...
        try:
            ...
            cur.execute("insert into movie(id,title,content,link) values (NULL ,'%s' ,'%s' ,'%s')" % (
                title, content.replace("'", r"\'"), link))  # 执行sql语句
            conn.commit()
