...
class my_thread(threading.Thread):
    def __init__(self, each_time, each_url):
        super(my_thread, self).__init__ ()
        self.each_time = each_time
        self.each_url = each_url

    def run(self):
        f(self.each_time, self.each_url)
...
url = "http://quotes.money.163.com/f10/dbfx_000078.html"
...
thread_assemble = []
for (each_time, each_url) in zip(list_report_time, list_report_url):
    t = my_thread(each_time, each_url)
    t.start()
